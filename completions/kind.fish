...
function __kind_perform_completion
    ...
    if not type -q "$args[1]"
        # This can happen when "complete --do-complete kind" is called when running this script.
        __kind_debug "Cannot find $args[1]. No completions."
        return
    end
...
function __kind_prepare_completions
    ...
    if not set --query __kind_comp_commandLine
        ...
        set __kind_comp_commandLine (commandline -c)
    end
...
# The call to __kind_prepare_completions will setup both __kind_comp_results and __kind_comp_do_file_comp.
